...
NB_INLINE PyObject *func_create(Func &&func, Return (*)(Args...),
    ...
    static constexpr auto descr =
        const_name("(") +
        // (concat) parenthesized to suppress ADL: a bound arg type in a foreign
        // namespace defining its own `concat` (e.g. nlohmann::detail) must not
        // hijack this call. See nb_descr.h.
        (concat)(type_descr(
